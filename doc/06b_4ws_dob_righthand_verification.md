...
## 6. 与 06a §3 (A-std, B-std) 对比

| 位置 | 本文 (RH) | 06a §3 (std) | 差异 |
| --- | --- | --- | --- |
| $A$ 全部元素 | 同右 | 同左 | **完全一致** |
| $B_{11}$ | $+C_f/m$ | $+C_f/m$ | 一致 |
| $B_{12}$ | $+C_r/m$ | $+C_r/m$ | 一致 |
| $B_{13}$ | $-g$ | $+g$ | **唯一差异** |
| $B_{21}$ | $+C_fl_f/I_z$ | $+C_fl_f/I_z$ | 一致 |
| $B_{22}$ | $-C_rl_r/I_z$ | $-C_rl_r/I_z$ | 一致 |

**唯一差异在 $B_{13}$**，原因是 $\phi$ 正方向定义不同：

| | 本文（右手定则） | 06a §3 |
| --- | --- | --- |
| $\phi > 0$ 含义 | 左高右低（右手绕 $x$） | 重力分量指向 $v_y$ 正方向（向左） |
| 重力对 $\dot{v}_y$ | $-g\phi$（向右推） | $+g\phi$（向左推） |
| 等价关系 | $\phi_{\text{RH}} = -\phi_{\text{06a-std}}$ | |
...
| 文档 | 出发约定 | 到 06 形的变换 | $B_{13}$ 路径 |
| --- | --- | --- | --- |
| **06a** | $v_y$ 左正，$\phi$ 随 $v_y$ | $T$（状态）+ $S$（输入） | $(-1)(+g)(-1) = +g$ |
| **06b**（本文） | $v_y$ 左正，$\phi$ 右手定则 | 仅 $T$（状态） | $(-1)(-g) = +g$ |
| **06c** | $v_y$ 右正，直接推导 | 无需变换 | 直接得 $+g$ |

> 无论 $\phi$ 约定如何，变换到 06 文档的 $v_y$ 方向后，$B_{13} = +g$ 恒成立。差异仅存在于中间形式（$v_y$ 左正）中 $B_{13}$ 的符号。
